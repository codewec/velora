...
<template>
  <DevicePageShell :connection-id="connectionId" :id="id" active-tab="state">
    <template #default="{ state }">
      ...
      >
        <div class="space-y-4">
          <p class="text-sm uppercase tracking-[0.25em] text-slate-500">{{ t('app.state') }}</p>
          <pre
            class="overflow-x-auto rounded-2xl bg-slate-100 p-4 text-xs text-slate-700 dark:bg-slate-950/80 dark:text-slate-300"
