...
const activeTab = ref<'overview' | 'inventory' | 'health' | 'transport'>('overview')

const coordinatorRevision = computed(() => {
  return coordinatorRevisionLabel(bridgeInfo.value?.coordinator?.meta, t('app.unknown'))
})

const deviceHealthRows = computed(() => {
  return buildDeviceHealthRows(devices.value, bridgeHealth.value?.devices)
})

const statsByTypeMap = computed(() => statsByType(devices.value))
const statsByPowerSourceMap = computed(() => statsByPowerSource(devices.value, t('app.unknown')))
const statsByModelMap = computed(() => statsByModel(devices.value, t('app.unknown')))
const statsByVendorMap = computed(() => statsByVendor(devices.value, t('app.unknown')))

const tabs = computed<NavigationMenuItem[]>(() => [
  {
    label: t('infoPage.overviewTab'),
  ...
  {
    label: t('infoPage.inventoryTab'),
    active: activeTab.value === 'inventory',
    onSelect: () => {
      activeTab.value = 'inventory'
    },
  },
  {
    label: t('infoPage.healthTab'),
    active: activeTab.value === 'health',
    onSelect: () => {
      activeTab.value = 'health'
    },
  },
  {
    label: t('infoPage.transportTab'),
    active: activeTab.value === 'transport',
    onSelect: () => {
  ...
  <UDashboardPanel id="information">
    ...
    <template #body>
      <div class="space-y-6">
        ...
        <template v-else-if="activeTab === 'inventory'">
          <div class="grid gap-4 xl:grid-cols-4">
            ...
            <UCard class="border-default bg-default/70" :ui="{ body: 'p-4' }">
              ...
              <div class="mt-2 space-y-1 text-sm text-highlighted">
                <p v-for="(count, key) in statsByTypeMap" :key="key">{{ key }}: {{ count }}</p>
              </div>
            </UCard>
            ...
            <UCard class="border-default bg-default/70" :ui="{ body: 'p-4' }">
              ...
              <div class="mt-2 space-y-1 text-sm text-highlighted">
                <p v-for="(count, key) in statsByPowerSourceMap" :key="key">
                  {{ key }}: {{ count }}
                </p>
              </div>
            </UCard>
            ...
            <UCard class="border-default bg-default/70" :ui="{ body: 'p-4' }">
              ...
              <div class="mt-2 space-y-1 text-sm text-highlighted">
                <p v-for="(count, key) in statsByVendorMap" :key="key">{{ key }}: {{ count }}</p>
              </div>
            </UCard>
          </div>
          ...
          <UCard class="border-default bg-default/70" :ui="{ body: 'p-4' }">
            ...
            <div class="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-3">
              <p
                v-for="(count, key) in statsByModelMap"
                :key="key"
            ...
            <div class="grid gap-4 xl:grid-cols-3">
              <UCard class="border-default bg-default/70" :ui="{ body: 'p-4' }">
                ...
                <p class="mt-2 text-xs text-muted">
                  {{
                    t('infoPage.lastCheck') +
                    ': ' +
                    formatInfoDateTime(bridgeHealth.response_time, t('app.unknown'))
                  }}
                </p>
              </UCard>
              ...
              <UCard class="border-default bg-default/70" :ui="{ body: 'p-4' }">
                ...
                <p class="mt-2 text-base font-semibold text-highlighted">
                  {{
                    t('infoPage.uptime') +
                    ': ' +
                    formatInfoDuration(bridgeHealth.process.uptime_sec, t('app.unknown'))
                  }}
                </p>
                <p class="mt-2 text-base font-semibold text-highlighted">
        ...
        <template v-else-if="activeTab === 'transport'">
          <div class="grid gap-4 xl:grid-cols-3">
            <UCard class="border-default bg-default/70" :ui="{ body: 'p-4' }">
              ...
              <p class="mt-2 text-sm text-highlighted">
                ...
                {{
                  z2m.metrics.value.lastMessageTs
                    ? formatInfoDateTime(z2m.metrics.value.lastMessageTs, t('app.unknown'))
                    : t('app.unknown')
                }}
              </p>
              <p class="mt-1 text-sm text-muted">
                {{ t('infoPage.sent') }}: {{ z2m.metrics.value.messagesSent }} ·
                {{ formatInfoMb(z2m.metrics.value.bytesSent) }}
              </p>
              <p class="text-sm text-muted">
                {{ t('infoPage.received') }}: {{ z2m.metrics.value.messagesReceived }} ·
                {{ formatInfoMb(z2m.metrics.value.bytesReceived) }}
              </p>
            </UCard>
